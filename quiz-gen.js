...
`
  ...
  const JS = [
    ...
    "function toggleCat(c){const e=c.dataset.c;if(e==='ALLC'){document.querySelectorAll('#catBar .chip').forEach(x=>x.classList.add('on'));activeCats=new Set(Object.keys(CATS));}else{c.classList.toggle('on');if(c.classList.contains('on'))activeCats.add(e);else activeCats.delete(e);if(activeCats.size===0){c.classList.add('on');activeCats.add(e);}const a=document.querySelector('#catBar .chip[data-c=\"ALLC\"]');if(a)a.classList.toggle('on',activeCats.size===Object.keys(CATS).length);}refresh();}",
