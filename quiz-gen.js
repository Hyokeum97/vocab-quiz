...
`
  ...
  const JS = [
    ...
    "function loadState(){try{const r=localStorage.getItem(STORAGE_KEY);if(!r){badge('💾 새 세션 시작');return;}const d=JSON.parse(r);results=d.results||{};starred=new Set(d.starred||[]);badge('💾 기록 불러옴 · '+(d.savedAt?new Date(d.savedAt).toLocaleString('ko-KR'):'')+' 저장본');}catch(e){badge('💾 새 세션 시작');}}",
    "function saveState(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify({results,starred:[...starred],savedAt:new Date().toISOString()}));badge('💾 자동 저장됨 · '+new Date().toLocaleTimeString('ko-KR'));}catch(e){}}",
    ...
    "function mark(ok){results[deck[idx][0]]=ok?'correct':'wrong';if(ok){correct++;streak++;}else{wrong++;streak=0;}saveState();idx++;answered=false;flipped=false;if(ttsOk)speechSynthesis.cancel();renderQuiz();}",
    ...
    "function switchView(v){view=v;document.getElementById('tabList').classList.toggle('on',v==='list');document.getElementById('tabQuiz').classList.toggle('on',v==='quiz');document.getElementById('listView').classList.toggle('hidden',v!=='list');document.getElementById('quizView').classList.toggle('hidden',v!=='quiz');if(ttsOk)speechSynthesis.cancel();refresh();}",
